...
type TabId = 'backgrounds' | 'badges' | 'cat';
...
export default function ShopPage() {
    ...
    useEffect(() => {
        ...
        setLoading(true);
        if (tab === 'cat') {
            fetch('/api/shop?type=CAT_ITEM')
                .then((r) => r.json())
                .then((wear) => {
                    setItems(Array.isArray(wear) ? wear : []);
                    setLoading(false);
                })
    ...
    const catWearPurchases =
        user?.purchases?.filter((p) => p.item.type === 'CAT_ITEM') ?? [];
    const shopPreviewEquipped = equippedWearForActiveSkin(user?.catConfig);
    return (
            ...
            <main className="px-4 py-4 flex-1">
                ...
                {loading ? (
                    ...
                    <div className={tab === 'backgrounds'
                        ...
                        {tab === 'cat' && (
                            <div className="col-span-full bg-slate-900 rounded-[2.5rem] p-8 mb-4 border border-white/5 relative overflow-visible flex flex-col items-center">
                                ...
                                <div className="relative z-10 scale-105 py-2">
                                    <CatOleg
                                        equippedItemIds={shopPreviewEquipped}
                                        ownedItems={catWearPurchases}
                                        size="lg"
                                    />
                                </div>
                        ...
                        {tab === 'cat' && items.length > 0 && (
                            <div className="col-span-full -mt-2 mb-2 px-1">
                                <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">Одежда и аксессуары</p>
                            </div>
                        )}
